...
const cityId = (city: IranDatasetCityRecord): string =>
	`${city.state}|${city.province}|${city.city}|${city.latitude}|${city.longitude}`;
...
export class CityService {
	...
	async getSelectedCity(
		...
		legacyProvinceCode: string | undefined,
	): Promise<City> {
		...
		const legacy =
			legacyProvinceCode === "1"
				? cities.find(
						(city) => city.city === TEHRAN_CITY_NAME && city.province === TEHRAN_CITY_NAME,
					)
				: undefined;
	...
	getTehran(cities: City[]): City {
		return (
			cities.find((city) => city.city === TEHRAN_CITY_NAME && city.province === TEHRAN_CITY_NAME) ??
			cities[0]
		);
	}
	...
	private parseDataset(dataset: unknown): City[] {
		if (
			!isRecord(dataset) ||
			!isRecord(dataset.DocumentElement) ||
			!Array.isArray(dataset.DocumentElement.cities)
		) {
			return [];
		}
