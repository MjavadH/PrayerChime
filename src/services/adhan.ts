...
const solarDeclination = (eclipticLongitude: number): number => {
	return radiansToDegrees(
		Math.asin(Math.sin(degreesToRadians(eclipticLongitude)) * Math.sin(degreesToRadians(23.44))),
	);
};
...
const equationOfTime = (meanAnomaly: number, eclipticLongitude: number): number => {
	...
	const l0 = normalizeDegrees(
		eclipticLongitude -
			1.9148 * Math.sin(degreesToRadians(meanAnomaly)) -
			0.02 * Math.sin(degreesToRadians(2 * meanAnomaly)),
	);
	const e = 0.016708634;
	const value =
		y * Math.sin(2 * degreesToRadians(l0)) -
		2 * e * Math.sin(degreesToRadians(meanAnomaly)) +
		4 * e * y * Math.sin(degreesToRadians(meanAnomaly)) * Math.cos(2 * degreesToRadians(l0)) -
		0.5 * y * y * Math.sin(4 * degreesToRadians(l0)) -
		1.25 * e * e * Math.sin(2 * degreesToRadians(meanAnomaly));
	return radiansToDegrees(value) * 4;
};

const hourAngle = (latitude: number, declination: number, altitude: number): number => {
	const numerator =
		Math.sin(degreesToRadians(altitude)) -
		Math.sin(degreesToRadians(latitude)) * Math.sin(degreesToRadians(declination));
	const denominator =
		Math.cos(degreesToRadians(latitude)) * Math.cos(degreesToRadians(declination));
...
export class PrayerTimes {
	...
	constructor(
		...
		const center =
			1.9148 * Math.sin(degreesToRadians(meanAnomaly)) +
			0.02 * Math.sin(degreesToRadians(2 * meanAnomaly)) +
			0.0003 * Math.sin(degreesToRadians(3 * meanAnomaly));
